...
9. На экране ноды появится запрос с IP клиента. Если это ваше подключение, разрешите его кнопками. По умолчанию выбран отказ; без ответа запрос отклоняется через 30 секунд.
...
Не каждое приложение MeshCore поддерживает TCP. Наличие Wi-Fi в прошивке не добавляет этот способ подключения в приложение автоматически. Если приложение закрыло соединение, пока вы подтверждали запрос на ноде, повторите подключение и подтвердите новый запрос.
...
- Разрешённый Wi-Fi-клиент получает полный доступ через companion-протокол, включая изменение настроек. Подтверждайте только своё подключение. IP на экране сам по себе не удостоверяет личность клиента.

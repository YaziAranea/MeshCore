...
Изображения ниже — пиксельная симуляция интерфейса SmartUI PS17. Первые три платы сохраняют основной контракт `v2.0.0-rc1`; V4.3 и Wireless Paper добавлены в линии 2.1. Hotfix `2.1.0-dev.1` добавляет общую PIN-страницу всем пяти платам. В `2.1.0-dev.2` существующие шрифты/геометрия сохранены; GPIO и резонанс получили режим выбора с подтверждением, а пункт защиты АКБ показывает действующий порог. Это не фотографии и не подтверждение электрической проверки.

...
Главная карусель содержит только полезные и доступные данной плате страницы: часы, сеть, чат, анонс, настройки и выключение. До BLE-связи hotfix `2.1.0-dev.1` автоматически показывает отдельную крупную страницу активного BLE PIN на всех пяти платах; после подключения она скрывается.
...
На T096, T114, ProMicro RA62, V4.3 OLED и Wireless Paper используется один понятный сценарий: пока BLE не подключён, на экране крупно видны активные шесть цифр. Клиент или операционная система может называть их `PIN`, `passkey` или «кодом сопряжения». После связи PIN-страница исчезает из карусели.

Wireless Paper сохраняет PIN на e-paper при idle-переходах и не затирает его screensaver-экраном. После успешной связи код скрывается при следующем обновлении панели.

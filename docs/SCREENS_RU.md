...
Изображения ниже — симуляция эталонного интерфейса, перенесённая из стабильной PS16-линии. Они задают визуальный контракт PS17-порту, но не являются фотографиями и сами по себе не подтверждают работу `v2.0.0-rc1` на платах.
...
Эталонная PS16-линия дала `455 PASS / 0 FAIL`. Стресс-тест охватывал 0, 1 и 350 контактов, длинные русские имена, три страницы клавиатуры, 0, 1 и 12 отправителей ЛС, до 30 сохранённых ЛС, все публичные шрифты и состояния GPS T114. Для PS17 RC симуляцию необходимо запустить заново после объединения порта. Это результат модели интерфейса, а не сертификат электрической или радиочастотной части платы.

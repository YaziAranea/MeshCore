...
Изображения ниже — пиксельная симуляция интерфейса SmartUI PS17. Первые три платы сохраняют основной контракт `v2.0.0-rc1`; V4.3 и Wireless Paper добавлены в линии 2.1. В `v2.1.0-beta.2` аптайм находится под часами и показывает суммарные часы с минутами: `U 40h05m` — 40 часов 5 минут. T114 размещает рядом короткую дату `дд.мм`, компактные OLED используют часы 2×. Это не фотографии и не подтверждение электрической проверки.
...
В beta.2 пиктограммы одинаковы по смыслу на TFT, OLED и Paper. Их высота выравнивается по видимым пикселям букв, включая пустые поля внутри bitmap-шрифтов.

![Пиктограммы: до и после](assets/qa/ICON_BEFORE_AFTER_ALL_BOARDS.png)

...
> V3, V4.3 и Paper merged содержат подготовленную пустую SPIFFS и заменяют
...
- `merged.bin` записывается с адреса `0x00000`. Для всех трёх плат это **только чистая установка со сбросом данных**, не обычное обновление.
...
### V4.3 / Wireless Paper: подготовленная чистая установка

Их `merged.bin` содержит canonical formatted-empty SPIFFS точного размера из
реальной таблицы разделов: V4.3 — `0xC90000 + 0x360000`, Paper —
...
Файлы V3, V4.3 и Paper не взаимозаменяемы.

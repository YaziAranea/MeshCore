...
def production_code(source):
    ...
    assert release and '"V3"' in release.group(), "release-label fallback must be V3"
...
struct DisplayDriver {
  ...
  int getTextWidth(const char* text) const {
    assert(!strcmp(text,"MeshCore") || !strcmp(text,"V3"));
...
'''
    ...
    for font in range(count):
        for size in (1, 2):
            for bold in (False, True):
                ...
                b = metrics(kind, font, size, bold, "V3")
  ...
  assert(d.draws.size()==2 && d.draws[0].text=="MeshCore" && d.draws[1].text=="V3");
...
def render_splash_preview(kind, font, out=None):
    ...
    records = execute_cpp(kind, method, helpers, release, out or ROOT / "qa_outputs/splash-smartui-v3/legacy")[font]
...
def main(out, preview):
    ...
    for name, kind, width, height, physical_w, physical_h, fonts, default in BOARDS:
        for initial in fonts:
            ...
            label = records[1]
            _, label_top, label_ink, _ = metrics(kind, label["font"], label["size"], label["bold"], "V3")
            ...
            assert records[1]["text"] == "V3" and len(records) == 2
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "qa_outputs/splash-smartui-v3")
    parser.add_argument("--preview", type=Path, default=ROOT / "docs/assets/ui/boot-smartui-v3.png")

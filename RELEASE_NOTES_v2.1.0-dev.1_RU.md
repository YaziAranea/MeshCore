...
Hotfix-выпуск, обновлённый 2026-08-25, для пяти плат. Он заменяет предыдущий Release `v2.1.0-dev`; история старого выпуска [сохранена](RELEASE_NOTES_v2.1.0-dev_RU.md).
...
- Экран PIN больше не перехватывает запуск, входящее ЛС, запрос телеметрии, auto-home, пробуждение или e-paper screensaver.
- Нода запускается и возвращается на обычные часы; Wireless Paper снова показывает часы с лесным фоном в idle-режиме.
- PIN открывается вручную: с часов один переход вперёд. После успешной BLE-связи страница скрывается.
- Случайный код создаётся один раз после очистки/сброса и сохраняется между обычными перезагрузками.
- Убран дублирующий старый экран `FIRST`.

`123456` в build flags — не универсальный пароль. Если сохранённого PIN нет, прошивка создаёт случайный код и сохраняет его до следующей очистки/сброса.

...
Известное ограничение: нерелизная BLE-цель Heltec T1 этой ветки не помещается во flash. Её USB-вариант в CI проверяет общий UI/display-код без BLE, а не исправность BLE-сборки. RAK4631 не является релизной или обязательной compatibility-целью beta. Все шесть публикуемых конфигураций собираются отдельно; [подробности](docs/BUILD_RU.md#целевые-сборки).

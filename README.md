...
В кандидатах на выпуск `v2.0.0-rc1` страница версии показывает `T096 SmartUI 2.0.0-rc1`, `T114 SmartUI 2.0.0-rc1` или `ProMicro SmartUI 2.0.0-rc1`.
...
`v2.0.0-rc1` является предварительным выпуском. Не используйте будущие имена UF2 из документации, пока файлы и `SHA256SUMS.txt` не появились в одном GitHub Release.
...
В репозитории подготовлены воспроизводимые сборки и read-only CI ровно для трёх целей. Перед публикацией RC должны быть заново выполнены:

- сборка трёх целевых PlatformIO environments;
- статический контракт UI;
- точная framebuffer-симуляция;
- проверка UF2-блоков, magic values, адреса старта и маркеров версии.

T096 симулируется с реальными bitmap-метриками, T114 — через фактическое логическое масштабирование 128×64 → 240×135, ProMicro — по таблицам OLED 128×64. Приложенные изображения сохраняют эталон интерфейса PS16 и служат визуальным контрактом порта. Они не подтверждают работу PS17 RC на железе; отдельная аппаратная проверка `v2.0.0-rc1` не заявляется.

...
Текущий выпуск — `SmartUI 2.1.0-dev.2`: оптимизация памяти, подтверждение GPIO/резонанса перед сохранением, согласованная работа координат и защита АКБ. Предыдущий `v2.1.0-dev.1` сохранён для отката; RC-линия `v2.0.0-rc1` не изменена.

[⬇ Скачать SmartUI v2.1.0-dev.2](https://github.com/YaziAranea/MeshCore/releases/tag/v2.1.0-dev.2) · [Как выбрать файл](RELEASE_NOTES_v2.1.0-dev.2_RU.md#какой-файл-скачать) · [Инструкция по прошивке](docs/FLASHING_RU.md)
...
Используйте [Release v2.1.0-dev.2](https://github.com/YaziAranea/MeshCore/releases/tag/v2.1.0-dev.2). Он сохраняет предыдущие исправления зуммера, настроек, времени, ADC и ручного BLE PIN. Файлы прошлых выпусков остаются на своих страницах.
...
Результаты и размеры именно `v2.1.0-dev.2` публикуются в [примечаниях к выпуску](RELEASE_NOTES_v2.1.0-dev.2_RU.md). Проверки старого `v2.1.0-dev.1` сохранены в его [исторических примечаниях](RELEASE_NOTES_v2.1.0-dev.1_RU.md), а не выдаются за результаты новой сборки.

Read-only CI собирает шесть релизных конфигураций, проверяет UF2/BIN, native-тесты и точные модели дисплеев. Дополнительная матрица display-драйверов использует `Heltec_v3_companion_radio_ble`, `RAK_4631_companion_radio_usb`, `Xiao_S3_WIO_companion_radio_ble` и `Heltec_t1_companion_radio_usb`; это контроль совместимости, не добавление этих плат в Release.

Известное ограничение: нерелизные BLE-цели Heltec T1 и RAK4631 этой ветки не помещаются во flash. Их USB-варианты в CI проверяют общий UI/display-код без BLE, а не исправность BLE-сборок. Все шесть конфигураций опубликованного Release собираются; [подробности ограничения](docs/BUILD_RU.md#целевые-сборки).

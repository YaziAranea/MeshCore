...
> [!WARNING]
> **V3 FS1 исправляет чистый запуск после Erase / `STORAGE ERROR`.**
> Для уже очищенной V3 скачайте заново **merged FS1** и запишите по `0x0`.
...
> Имена файлов прежние: проверяйте свежие SHA256 и ревизию `FS1` в V3-манифесте.
...
2. Скачайте актуальные контрольные суммы: `SHA256SUMS-V3.txt` для V3 FS1, `SHA256SUMS-ESP32.txt` для исходных V4.3/Paper, `SHA256SUMS.txt` для UF2. В общем архиве шести плат используйте `SHA256SUMS-ALL-SIX.txt`.
...
5. На nRF52 скопируйте UF2 на USB-диск. На работающей V3/V4.3/Wireless Paper используйте `update.bin` по адресу `0x10000` без Erase. Для чистой установки V3 после очистки нужен **merged FS1** по `0x0`: он сбрасывает данные. Ограничения чистой установки прежних V4.3/Paper описаны в [инструкции](docs/FLASHING_RU.md).
...
- Исключение — **V3 freshInstall-merged FS1**: его пустое SPIFFS перезаписывает identity/настройки даже без Erase. Для сохранения данных используйте update. V4.3/Wireless Paper в FS1 не пересобраны; после Erase их пустое хранилище также может вызвать `STORAGE ERROR`. V3-файлы им не подходят.
